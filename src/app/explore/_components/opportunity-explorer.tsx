...
export function OpportunityExplorer() {
  ...
  // Debounce department 400ms to avoid a query on every keystroke
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  const { data: detail, isFetching: detailFetching } = api.opportunities.getById.useQuery(
    { id: selectedId! },
    { enabled: !!selectedId },
  );
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex h-6 items-center justify-between">
        ...
        {isFetching && (
          <span className="animate-pulse text-xs text-muted-foreground">Refreshing…</span>
        )}
      </div>
